...
public interface ViralLoaderRepository extends CrudRepository<ViralLoaderEntity,Integer> {
    ...
    @Query(value = "SELECT RequestingDistrictName as requestingDistrictName,RequestingFacilityCode healthFacilityLabCode, TypeOfResult AS typeOfResult,RequestingFacilityName as facilityName, AnalysisDateTime as processingDate,COUNT(*) as totalReceived, " +
                ...
                " from VlData where RequestingFacilityCode in (:ouCodes) AND  CREATED_AT BETWEEN :startDateTime AND :endDateTime AND ENTITY_STATUS='ACTIVE' group by RequestingDistrictName,RequestingFacilityCode, TypeOfResult",nativeQuery = true)
    ...
    @Query(value = "select VLPendente.requestingDistrictName,VLPendente.healthFacilityLabCode,VLPendente.facilityName,VLPendente.totalPending " +
            ...
            "   from VlData where RequestingFacilityCode in (:ouCodes) AND VIRAL_LOAD_STATUS='PENDING' AND  DATEDIFF(CURRENT_TIMESTAMP, CREATED_AT)>2 AND ENTITY_STATUS = 'ACTIVE' group by RequestingDistrictName,RequestingFacilityCode ) VLPendente " +
