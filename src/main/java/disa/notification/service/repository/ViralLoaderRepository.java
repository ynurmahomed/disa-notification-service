...
public interface ViralLoaderRepository extends CrudRepository<ViralLoaderEntity,Integer> {
    ...
    @Query(value = "SELECT RequestingDistrictName as requestingDistrictName,RequestingFacilityCode healthFacilityLabCode, TypeOfResult AS typeOfResult,RequestingFacilityName as facilityName, AnalysisDateTime as processingDate,COUNT(*) as totalReceived, " +
                ...
                " from VlData where RequestingFacilityCode in (:ouCodes) AND  CREATED_AT BETWEEN :startDateTime AND :endDateTime AND ENTITY_STATUS='ACTIVE' AND TypeOfResult IN ('HIVVL','CD4') group by RequestingDistrictName,RequestingFacilityCode, TypeOfResult",nativeQuery = true)
    ...
    @Query(value = "SELECT RequestId as requestId, UNIQUEID as nid, FIRSTNAME as firstName, SURNAME as lastName,RequestingDistrictName as requestingDistrictName,RequestingFacilityName as requestingFacilityName," +
            ...
            " from VlData where  RequestingFacilityCode in (:ouCodes) AND  CREATED_AT BETWEEN :startDateTime AND :endDateTime AND ENTITY_STATUS='ACTIVE' AND TypeOfResult IN ('HIVVL','CD4')",nativeQuery = true)
    ...
    @Query(value = "SELECT RequestId as requestId, UNIQUEID as nid, FIRSTNAME as firstName, SURNAME as lastName,RequestingDistrictName as requestingDistrictName,RequestingFacilityName as requestingFacilityName," +
            ...
            " from VlData where  RequestingFacilityCode in (:ouCodes) AND VIRAL_LOAD_STATUS='PENDING' AND  DATEDIFF(CURRENT_TIMESTAMP, CREATED_AT)>2 AND ENTITY_STATUS='ACTIVE' AND TypeOfResult IN ('HIVVL','CD4')",nativeQuery = true)
